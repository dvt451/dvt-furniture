 ...
 */
    ...
    (() => {
        ...
        for (let index = 0; index < menu__lists.length; index++) {
            ...
            let menuIndex = menuListItems.closest(".header__menu") ? 2 : 0;
            if (document.querySelectorAll(".header__menu .menu__list")) for (let i = menuIndex; i < menuItems.length; i++) {
...
***************************************************************************** */
        ...
        const galleries = document.getElementById("lg-swipper");
        const swiper = new Swiper(".swiper", {
            ...
            on: {
                init: function() {
                    if (galleries) {
                        const lg = lightgallery_es5(galleries, {
                            plugins: [ lg_thumbnail_min ],
                            download: false
                        });
                        galleries.addEventListener("lgBeforeClose", (() => {
        ...
        const galleriesArray = document.querySelectorAll(".animated-thumbnails");
        if (galleriesArray.length) {
            let galleyItems = [];
        ...
        if (document.querySelector("[data-tabs]")) {
            ...
            for (let i = 0; i < tabTitles.length; i++) {
                ...
                if (tabTitle.classList.contains("_active")) tabContents[i].style.display = "block";
                tabTitle.onclick = () => {
                    for (let ind = 0; ind < tabContents.length; ind++) {
                        const tabContent = tabContents[ind];
                        tabTitles[ind].classList.remove("_active");
        ...
        if (document.querySelector("[data-tabs-closed]")) {
            ...
            for (let i = 0; i < tabTitles.length; i++) {
                const tabTitle = tabTitles[i];
                if (tabTitle.classList.contains("_tab-active")) tabContents[i].classList.add("_active");
                tabTitle.onclick = () => {
                    for (let ind = 0; ind < tabContents.length; ind++) {
                        const tabContent = tabContents[ind];
                        if (!tabContents[i].classList.contains("_active")) {
                            tabContent.classList.remove("_active");
                            tabTitles[ind].classList.remove("_tab-active");
                        }
                    }
                    tabTitle.classList.toggle("_tab-active");
                    tabContents[i].classList.toggle("_active");
                };
            }
        }
        isWebp();
